...
describe('.env loading', () => {
  it('reads a key, ignores comments and blank lines, and strips quotes', () => {
    delete process.env['PLOUTOS_TEST_A'];
    delete process.env['PLOUTOS_TEST_B'];
    loadEnv(envFile('# a comment\n\nPLOUTOS_TEST_A=plain-value\nPLOUTOS_TEST_B="quoted value"\n'));
    expect(process.env['PLOUTOS_TEST_A']).toBe('plain-value');
    expect(process.env['PLOUTOS_TEST_B']).toBe('quoted value');
  });
  ...
  it('never overrides something already in the environment', () => {
    ...
    process.env['PLOUTOS_TEST_C'] = 'from-the-shell';
    loadEnv(envFile('PLOUTOS_TEST_C=from-the-file\n'));
    expect(process.env['PLOUTOS_TEST_C']).toBe('from-the-shell');
  });
  ...
  it('leaves an empty assignment unset rather than setting an empty key', () => {
    ...
    delete process.env['PLOUTOS_TEST_D'];
    loadEnv(envFile('PLOUTOS_TEST_D=\n'));
    expect(process.env['PLOUTOS_TEST_D']).toBeUndefined();
  });
});
